...
export default function ArcEdge({ arc, places, transitions, selected, showHandle, onClick, onCpMouseDown }: Props) {
  ...
  // Connection points aimed toward control point (gives natural bezier tangent)
  ...
  // Shorten endpoint for inhibitor circle tip
  let ex = x2, ey = y2;
  if (isInhibitor) {
  ...
  // Bezier midpoint at t=0.5: 0.25*P0 + 0.5*P1 + 0.25*P2
  ...
  return (
    <g>
      ...
      <path
        ...
        markerEnd={isInhibitor ? undefined : 'url(#arrow-end)'}
        onClick={onClick} className="cursor-pointer"
      />
      {isInhibitor && (
      ...
      {(arc.weight > 1 || arc.type === 'reset') && (
        <text x={labelX} y={labelY} fontSize={11} fill={color} textAnchor="middle" className="select-none pointer-events-none">
          {arc.type === 'reset' ? 'R' : arc.weight}
        </text>
      )}

      {/* Control-point drag handle – only when arc is selected */}
      {showHandle && (
        <g
          transform={`translate(${cp.x},${cp.y})`}
          onMouseDown={onCpMouseDown}
